...
/**
 * Reports results from TCK as tabulated text, suitable for dumping to the
 * console or a file and being read by a human.
 * 
 * If can be reused to provide a summary reports of different candidates as long
 * as the same objenesisDescription is not used twice.
 * 
 * @author Joe Walnes
...
public class TextReporter implements Reporter {
	...
	public void result(boolean instantiatedObject) {
		...
		((Map) objenesisResults.get(currentObjenesis)).put(currentCandidate,
				new Result(currentObjenesis, currentCandidate,
	...
	public void exception(Exception exception) {
		...
		((Map) objenesisResults.get(currentObjenesis))
				.put(currentCandidate, new Result(currentObjenesis,
						currentCandidate, false, exception));
	}
